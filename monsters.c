...
int
creat_mons(struct thing *person, int monster, int report)
...
void
genmonsters(int least, int treas)
{
    reg int i;
...
new_monster(struct linked_list *item, int type, coord *cp, int max_monster)
{
    ...
    if (vlevel > HARDER) 
	 tp->t_stats.s_intel = tp->t_stats.s_intel + ((vlevel - HARDER)/2);
...
int
randmonster(int wander, int no_unique)
{
...
wake_monster(int y, int x)
{
    ...
    register char *mname;
    int nasty;	/* Will the monster "attack"? */
    char ch;
	...
	if(on(*tp, CANSNORE) &&  
	   ...
	   !save(VS_PARALYZATION, &player, 0)) {
		...
		if (rnd(100) < 90) turn_off(*tp, CANSNORE);
	    }
	}
	...
	if (on(*tp, LOOKSTONE)) {
		...
		msg("The gaze of the %s has no effect.", monsters[tp->t_index].m_name);
...
wanderer()
{
    ...
    register struct linked_list *item;
    register struct thing *tp;
    register unsigned int *attr;	/* Points to monsters' attributes */
    int carry;	/* Chance of wanderer carrying anything */
    int rmonst;	/* Our random wanderer */
    int canteleport = FALSE;	/* Can the monster teleport? */
	int seehim;	/* Is monster within sight? */
    coord cp;
