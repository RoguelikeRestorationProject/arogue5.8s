...
can_blink(struct thing *tp)
{
	...
	for (x=hero.x-1; x<hero.x+2; x++, index++) {
	     ...
	     */
	    if (x<0 || x >= MY_COLS || index == 4) continue;
 ...
 */
{
    int damage, dist, thisdist, monst_dist = INT_MAX; 
	...
	*mdead = 0;
	...
	for (x = er->x - 1; x <= ex; x++) {
	    if (x < 0 || x >= MY_COLS) /* Don't try off the board */
		continue;
	    for (y = er->y - 1; y <= ey; y++) {
		coord tryp;

		if ((y < 1) || (y >= MY_LINES - 2)) /* Don't try off the board */
		    continue;
...
/* flee = True if running away or player is inaccessible in wall */
{
    ...
    if (on(*th, ISSHK) && not_paid > 0) {
	if (hero.x != shop_door.x || hero.y != shop_door.y) {
		th->t_dest = &shop_door;
	}
	else {
		th->t_dest = &shk_pos;
	...
	(off(*th, CANFALL) || ISWEARING(R_ALERT)))
		running = FALSE;
...
get_hurl(struct thing *tp)
{
    ...
    int bow=FALSE, crossbow=FALSE, sling=FALSE;
