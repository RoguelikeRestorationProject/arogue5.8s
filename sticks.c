 ...
 */
int
do_zap(int gotdir, int which, int flag)
{
	...
	case WS_LIGHT:
	    ...
	    if (is_stick) ws_know[WS_LIGHT] = TRUE;
	when WS_DRAIN:
	    /*
	    ...
	    if (is_stick) ws_know[WS_DRAIN] = TRUE;
	when WS_POLYMORPH:
	case WS_TELMON:
	case WS_CANCEL:
	{
	...
	when WS_MISSILE:
	{
	    ...
	    if (is_stick) ws_know[WS_MISSILE] = TRUE;
	}
	when WS_HIT:
	{
	    register char ch;
	    ...
	    if (is_stick) ws_know[WS_HIT] = TRUE;
	}
	when WS_MAKEINVIS: {
	    y = hero.y;
	    x = hero.x;
	    ...
	    {
	    	if (is_stick) ws_know[WS_MAKEINVIS] = TRUE;
		item = find_mons(y, x);
		tp = THINGPTR(item);
		turn_on(*tp, ISINVIS);
		if (isalpha(mvwinch(cw, y, x)))
	...
	when WS_SLEEP: {
		...
		if (off(*tp, ISUNDEAD) && off(*tp, NOSNORE) && !save(VS_MAGIC, tp, 0)) {
	    		if (is_stick) ws_know[WS_SLEEP] = TRUE;
			msg("The %s seems to have fallen asleep.", monsters[tp->t_index].m_name);
	...
	when WS_AFFECT:
	    {
			...
			if (on(*tp, ISSHK)) {
    			...
    			strcpy(mname, monsters[tp->t_index].m_name);
			if ((tp->t_index == NUMMONST) && (off(*tp, CANSELL))) {
				strcat(mname, " ");
				strcat(mname, shkname);
			}
			...
			annoy:
    			...
    			/* Annoy monster */
   			if (off(*tp, WASTURNED)) runto(tp, &hero);
		}
	    }
	when WS_CHARGE:
	    if (ws_know[WS_CHARGE] != TRUE && is_stick)
		msg("This is a wand of charging.");
	    if (is_stick) ws_know[WS_CHARGE] = TRUE;
	...
	case WS_DEATH:
		...
		if (which == WS_ELECT)
		    ...
		    name = "flame";
		else if (which == WS_DEATH)
		    name = "death ray";
		else
		    name = "ice";

	        if (is_stick) ws_know[which] = TRUE;
	...
	when WS_CONFMON:
	    ...
	    if (is_stick) ws_know[WS_CONFMON] = TRUE;
	when WS_PARALYZE:
	    if (cursed) {
		...
		if (isalpha(mvwinch(mw, y, x)))
		{
		    ...
		    tp = THINGPTR(item);
		    runto(tp, &hero);
	            if (is_stick) ws_know[WS_FEAR] = TRUE;
	...
	when WS_MDEG:
		...
		item = find_mons(y, x);
		tp = THINGPTR(item);
	        if (is_stick) ws_know[WS_MDEG] = TRUE;
		if (on(*tp, ISSHK)) {
			msg("%s becomes angry.", shkname);
			not_paid++;
			turn_off(*tp, ISSHK);
	...
	when WS_DISINTEGRATE:
		...
		item = find_mons(y, x);
		tp = THINGPTR(item);
	        if (is_stick) ws_know[WS_DISINTEGRATE] = TRUE;
		if (on(*tp, ISSHK)) {
			msg("%s becomes angry.", shkname);
			not_paid++;
			turn_off(*tp, ISSHK);
...
shoot_bolt(struct thing *shooter, coord start, coord dir, int get_points, int reason, char *name, int damage)
{
	...
	switch (ch)
	{
		...
		if (dirch == '-' || dirch == '|') {
		    ...
		    int anychange = FALSE; /* Did we change anthing */
		...
		if (isalpha(ch)) {
		    ...
		    register struct thing *tp;
		    register char *mname;
		    int see_monster = cansee(pos.y, pos.x);
			...
			if (see_monster) {
			    ...
			    msg("The %s hits the %s", name, mname);
			}
			 ...
			 */
				...
				if (see_monster)
				    msg("The %s kills the %s", name, mname);
				else
			...
			if (see_monster) {
				...
				msg("The %s whizzes past the %s", name, mname);
			}
