 ...
 */
void
create_obj(int prompt, int which_item, int which_type)
{
    ...
    */
	...
	if (wmi != NULL) {
		...
		if(wmi->mi_prob == old_prob && wizard == FALSE) { 
		    ...
		    mvwaddch(hw,ii % 13,ii > 12 ? MY_COLS/2 : 0, ii + 'a');
		    waddstr(hw,") ");
	...
	mvwaddstr(hw,MY_LINES - 1, 0, prbuf);
	draw(hw);
	do {
	    ch = wgetch(hw);
	    if (ch == ESCAPE) {
 ...
 */
int
makemonster(int create) 
{
    register int i;
    register int which_monst;
    register int num_monst = NUMMONST, pres_monst=1, num_lines=2*(MY_LINES-3);
    char monst_name[40];
...
teleport()
{
    ...
    if (new_rp != NULL && levtype != MAZELEV) {
