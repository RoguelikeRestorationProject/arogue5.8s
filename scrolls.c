...
genocide()
{
	...
	for (i=0; i<left_limit && pres_monst<=NUMMONST-NUMUNIQUE-1; i++) {
	    sprintf(monst_name, 
		    ...
		    monsters[pres_monst].m_normal ? ' ' : '*',
		    monsters[pres_monst].m_name);
	    wmove(hw, i+2, MY_COLS/2);
	    waddstr(hw, monst_name);
	    pres_monst++;
	}

	if ((num_monst -= num_lines) > 0) {
	    mvwaddstr(hw, MY_LINES-1, 0, morestr);
	    draw(hw);
	    wait_for(hw,' ');
	}
...

void
read_scroll(int which, int flag, int is_scroll)
{
	...
	cursed = (flag & ISCURSED) != 0;
	blessed = (flag & ISBLESSED) != 0;
    }
	...
	case S_CONFUSE:
	    ...
	    if (is_scroll) s_know[S_CONFUSE] = TRUE;
	when S_CURING:
	    /*
	...
	when S_HOLD:
		 ...
		 */
			...
			for (y = hero.y-2; y <= hero.y+2; y++) {
			    if (y < 1 || x < 0 || y > MY_LINES - 3 || x > MY_COLS - 1)
				continue;
	    ...
	    if (is_scroll) s_know[S_HOLD] = TRUE;
	when S_NOTHING: {
	    s_know[S_NOTHING] = TRUE;
	    msg("This scroll appears to be blank.");
	}
	when S_AMNESIA: {
	    int c;

	    msg("Thinking of Maud you forget everything else.");
	    for (c=0;c<MAXSCROLLS;c++) {
		s_know[c] = FALSE;
		s_guess[c] = NULL;
	    }
	    for (c=0;c<MAXPOTIONS;c++) {
		p_know[c] = FALSE;
		p_guess[c] = NULL;
	    }
	    for (c=0;c<MAXRINGS;c++) {
		r_know[c] = FALSE;
		r_guess[c] = NULL;
	    }
	    for (c=0;c<MAXSTICKS;c++) {
		ws_know[c] = FALSE;
		ws_guess[c] = NULL;
	    }
	    for (c=0;c<MAXMM;c++) {
	...
	when S_PUNISHMENT: {
	    ...
	    s_know[S_PUNISHMENT] = TRUE;
	    if (!punished) {
	    	msg("You are being punished for your misbehavior.");
	    	rnd_no = rnd(6);
		punish_dext = (max_stats.s_dext-rnd_no < 3 ? max_stats.s_dext-3 : rnd_no);
		max_stats.s_dext -= punish_dext;
		pstats.s_dext = pstats.s_dext - punish_dext;
	    	rnd_no = rnd(6);
	    	punish_str = (max_stats.s_str-rnd_no < 3 ? max_stats.s_str-3 : rnd_no);
		max_stats.s_str -= punish_str;
		pstats.s_str -= punish_str;
		turn_on(player, ISSLOW);
		player.t_turn = TRUE;
	    	fuse(unpunish, 0, PUNISHTIME, AFTER);
	    	punished = TRUE;
	    }
	    else {
		msg("The heavy iron ball gets heavier.");
		rnd_no = rnd(4);
		punish_dext += (max_stats.s_dext-rnd_no < 3 ? max_stats.s_dext-3 : rnd_no);
		max_stats.s_dext -= punish_dext;
		pstats.s_dext -= punish_dext;
		rnd_no = rnd(4);
		punish_str += (max_stats.s_str-rnd_no < 3 ? max_stats.s_str-3 : rnd_no);
		max_stats.s_str -= punish_str;
		pstats.s_str -= punish_str;
	    	lengthen(unpunish, PUNISHTIME);
	    }
	}
	when S_FIRE: {
	    s_know[S_FIRE] = TRUE;
	    msg("The scroll erupts in a tower of fire.");
	    if (ISWEARING(R_FIRE)) {
		msg("You are not injured.");
		r_know[R_FIRE] = TRUE;
	    }
	    else if (cur_gloves == NULL) {
		msg("Your hands get severely burned.");
		msg("You can't hold a weapon.");
		cur_weapon = NULL;
		noholdweap = TRUE;
		fuse(noholdweapon, 0, NOWEAPONTIME, AFTER);
	    }
	    else if (cur_gloves->o_flags & ISCURSED) {
		msg("Your hands get severely burned.");
		msg("You can't hold a weapon.");
		cur_weapon = NULL;
		noholdweap = TRUE;
		fuse(noholdweapon, 0, 2*NOWEAPONTIME, AFTER);
	    }
	    else
		msg("Your gloves protect you from injury.");
	}
	when S_SLEEP:
	    /*
	...
	when S_CREATE:
	     ...
	     * otherwise give up
	     */
	    creat_mons(&player, 0, TRUE);
	    light(&hero);
	    if (is_scroll) s_know[S_CREATE] = TRUE;
	when S_IDENT:
	    /* 
...
pet_message:	    msg("The dungeon begins to rumble and shake!");
		    ...
		    turn_on(player, ISINWALL);
	            if (is_scroll) s_know[S_PETRIFY] = TRUE;
		    break;
		default:
		    msg(nothing);
	    }
	...
	when S_ALLENCH: {
		...
		switch(lb->o_type) {
			...
			if (lb==cur_ring[LEFT_1]  || lb==cur_ring[LEFT_2]  ||
			    ...
			    switch (lb->o_which) {
				case R_ADDWISDOM: pstats.s_wisdom = pstats.s_wisdom + howmuch;
				when R_ADDINTEL:  pstats.s_intel = pstats.s_intel + howmuch;
				when R_ADDSTR:    pstats.s_str = pstats.s_str + howmuch;
				when R_ADDHIT:    pstats.s_dext = pstats.s_dext + howmuch;
			    }
			}
