...
be_trapped(struct thing *th, coord *tc)
{
    ...
    if ((is_player) && (ISWEARING(R_FLOAT) || on(player, ISFLOAT)))
	return '\0';
...
blue_light(int blessed, int cursed)
{
	...
	if (blessed) {
		...
		rm_turn_off(&(rooms[i]), ISDARK);
	}
	else if (rp) rm_turn_off(rp, ISDARK);
    }
...
corr_move(int dy, int dx)
{
    ...
    if (nh.x >= 0 && nh.x < MY_COLS && nh.y > 0 && nh.y < MY_LINES - 2) {
...
do_move(int dy, int dx)
{
     ...
     */
    if (nh.x < 0 || nh.x > MY_COLS-1 || nh.y < 1 || nh.y >= MY_LINES - 2
    ...
    old_heavy_ball = heavy_ball; /* save old heavy ball's position */
    ...
    if (rp != NULL && levtype != MAZELEV) {
    	if ((rm_on(rp, ISSHOP)) && (!in_shop) && (not_paid <= 0)) {
		if (first_time)
	...
	rm_turn_on(orp, FORCEDARK);     /* Fake darkness */
	light(&old_hero);
	rm_turn_off(orp, FORCEDARK);    /* Restore light state */
    }
    ...
    heavy_ball = old_hero;
    if (punished) {
    	ch = CCHAR( winat(old_heavy_ball.y, old_heavy_ball.x) );
    	wmove(cw,unc(old_heavy_ball));
    	waddch(cw, ch);
    }
    ch = CCHAR( winat(heavy_ball.y, heavy_ball.x) );
    wmove(cw, unc(heavy_ball));
    if (!isalpha(ch))
    	waddch(cw, punished ? HEAVY_BALL : ch);
    else
	waddch(cw, ch);
    wmove(cw, unc(hero));
    waddch(cw, PLAYER);
 ...
 */
int
getdelta(int match, int *dy, int *dx)
{
    register int y, x;
...
lit_room(struct room *rp)
{
    ...
    if (rm_off(rp, ISDARK)) return(TRUE);  /* definitely lit room */
...
show(int y, int x)
{
    register int ch = CCHAR( winat(y, x) );
	...
	/* Hide invisible creatures */
	    ...
	    if (off(player,CANSEE) || on(*tp,CANFALL))
		ch = CCHAR( mvwinch(stdscr, y, x) ); /* Invisible */
	}
