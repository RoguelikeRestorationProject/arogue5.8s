...
affect()
{
    ...
    /* Anything there? */
    if (new_pos.y < 0 || new_pos.y > MY_LINES-3 ||
	new_pos.x < 0 || new_pos.x > MY_COLS-1 ||
	...
	unsigned int test;	/* Overflow test */
...
steal()
{
	...
	int count = 0;
	unsigned int test;	/* Overflow check */
