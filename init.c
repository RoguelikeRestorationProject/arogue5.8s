...
"white",		"yellow", 		"bubbly",	
"glowing",		"smoky",		"puce",
"swirly",		"purple-red",		"ruby",		
"dark green", 		"emerald",		"sky blue",
"brilliant",		"clear",		"ebony",
};
...
char *stones[NSTONES] = {
	...
	"turquoise",		"zircon", 		"gold",
	"copper", 		"silver",		"granite",
	"wire",			"iron",			"ivory",
	"blackened",		"wooden",		"shining",
	"black Onyx",
};
...
init_names()
{
    ...
    {
	cp = prbuf;
	nwords = rnd(MY_COLS/20) + 1 + (MY_COLS > 40 ? 1 : 0);
	while(nwords--)
	{
	    nsyl = rnd(3)+1;
	    while(nsyl--)
...
init_things()
{
    ...
    for (c=0;c<=NUMMONST;c++)
	mon_know[c] = FALSE;
    mon_know[NUMMONST] = TRUE;
}
