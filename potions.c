...
add_dexterity(int cursed)
{
    ...
    pstats.s_dext = pstats.s_dext - ring_str;
...
add_intelligence(int cursed)
{
    ...
    pstats.s_intel = pstats.s_intel - ring_str;
...
add_wisdom(int cursed)
{
    ...
    pstats.s_wisdom = pstats.s_wisdom - ring_str;
...
void
quaff(int which, int flag, int is_potion)
{
	...
	case P_CLEAR:
		...
		if (is_potion) p_know[P_CLEAR] = TRUE;
	    }
	when P_HEALING:
	    if (cursed) {
	...
	when P_SEEINVIS:
	    ...
	    if (is_potion) p_know[P_SEEINVIS] = TRUE;
	when P_BOOZE: {
	    if (!save(VS_MAGIC, &player, 0)) {
		if (off(player, ISCLEAR)) {
		    if (on(player, ISHUH))
			lengthen(hangover, rnd(20)+HUHDURATION);
		    else {
			fuse(hangover, 0, rnd(20)+HUHDURATION, AFTER);
	                msg("Ooph!  This tastes like liquid fire!");
			turn_on(player, ISHUH);
		    }
		}
		else msg("You feel giddy for a moment, but it quickly passes.");
	    }
	    else
		msg("Nothing happens.");
    	    if (is_potion) p_know[P_BOOZE] = TRUE;
	}
	when P_FRUITJUICE: {
	    msg("This potion tastes like fruit juice.");
    	    food_left += HUNGERTIME + rnd(400) - 200;
    	    if (is_potion) p_know[P_FRUITJUICE] = TRUE;
	    }
	when P_PARALYSIS: {
	    if (!save(VS_MAGIC, &player, 0)) {
		msg("Your feet are frozen to the floor.");
		no_command = 4*FREEZETIME;
	    }
	    else
		msg("Nothing happens.");
    	    if (is_potion) p_know[P_PARALYSIS] = TRUE;
	    }
	when P_PHASE:
	    if (cursed) {
		msg("You can't move.");
	    ...
	    if (is_potion) p_know[P_PHASE] = TRUE;
	when P_FLY: {
	    int duration;
	    int say_message;
	...
	when P_RAISE:
	    ...
	    if (is_potion) p_know[P_RAISE] = TRUE;
	when P_HASTE:
	...
	when P_RESTORE: {
	    ...
	    if (is_potion) p_know[P_RESTORE] = TRUE;
	}
	when P_INVIS:
	    ...
	    if (is_potion) p_know[P_INVIS] = TRUE;
...
res_dexterity(int howmuch)
{
	...
	if (howmuch == 0)
	    ...
	    pstats.s_dext = min(pstats.s_dext+howmuch - punish_dext, max_stats.s_dext - punish_dext);
    }
