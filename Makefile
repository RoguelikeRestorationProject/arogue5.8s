...
DISTNAME=arogue5.8s.beta1
PROGRAM=arogue58s

O=o

HDRS  = rogue.h mach_dep.h network.h mdport.h
...
OBJS2 = pack.$(O) passages.$(O) player.$(O) potions.$(O) rings.$(O) rip.$(O) \
        rogue.$(O) rooms.$(O) save.$(O) scrolls.$(O) shops.$(O) state.$(O) \
	sticks.$(O) things.$(O) trader.$(O) util.$(O) vers.$(O) weapons.$(O) \
	wear.$(O) wizard.$(O) xcrypt.$(O)
OBJS  = $(OBJS1) $(OBJS2)
CFILES= \
      ...
      potions.c rings.c rip.c rogue.c rooms.c save.c scrolls.c shops.c \
      state.c sticks.c things.c trader.c util.c \
      weapons.c wear.c wizard.c xcrypt.c

MISC=	Makefile LICENSE.TXT arogue58s.sln arogue58s.vcproj
DOCS= arogue58s.doc arogue58s.html
