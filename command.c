	...
	else
	{
		...
		when 't':
			...
			if (delta.x == 0 && delta.y == 0) {
				msg("Don't throw things at yourself.");
				after = FALSE;
			}
			else
...
#ifdef WIZARD
		when CTRL('P') :
		    waswizard = 1; /* FIXME: add a password */
		    after = FALSE;
		    if (wizard)
		    {
			wizard = FALSE;
		...
		otherwise :
		    ...
		    if (wizard) switch (ch)
		    {
			case CTRL('B') : create_obj(TRUE, 0, 0);
...
void
search(int is_thief, int door_chime)
{
    coord found_loc;	/* the coordinate location */
    register int x_loc, y_loc;
    register int ch,	/* The trap or door character */
    ...
    if (on(player, ISBLIND))
	return;
    for (x_loc = hero.x - 1; x_loc <= hero.x + 1; x_loc++)
	for (y_loc = hero.y - 1; y_loc <= hero.y + 1; y_loc++)
	{
	    if (y_loc==hero.y && x_loc==hero.x)
		continue;

	    /* Mch and ch will be the same unless there is a monster here */
	    mch = CCHAR( winat(y_loc, x_loc) );
	    ch = CCHAR( mvwinch(stdscr, y_loc, x_loc) );
	    sch = CCHAR( mvwinch(cw, y_loc, x_loc) );	/* What's on the screen */

	    if (door_chime == FALSE && isatrap(ch)) {
		    register struct trap *tp;

		    /* Is there a monster on the trap? */
		    if (mch != ch && (item = find_mons(y_loc, x_loc)) != NULL) {
			mp = THINGPTR(item);
		    ...
		    if (mp != NULL && CCHAR(mvwinch(cw, y_loc, x_loc)) == mch)
			mp->t_oldch = ch; /* Will change when monst moves */
		    else mvwaddch(cw, y_loc, x_loc, ch);
...
d_level()
{
    ...
    if (ISWEARING(R_FLOAT) || on(player, ISFLY)) {
	msg("You can't go down stairs while floating in the air!");
	return;
    }
