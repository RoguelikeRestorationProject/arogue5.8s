...
static struct matrix att_mat[5] = {
...
fight(coord *mp, struct object *weap, int thrown)
{
	...
	if (!ISWEARING(R_ALERT)) {
		if (cur_helmet == NULL) {
			msg("The %s falls from the ceiling!", monsters[tp->t_index].m_name);
			msg("You are knocked unconscious.");
			no_command = FREEZETIME;
			did_hit = thrown;
		}
		else if (cur_helmet->o_flags & ISCURSED) {
			msg("The %s falls from the ceiling!", monsters[tp->t_index].m_name);
			msg("You are knocked unconscious.");
			no_command = 2*FREEZETIME;
			did_hit = thrown;
		}
		else {
			msg("The %s falls from the ceiling!", monsters[tp->t_index].m_name);
			msg("Your helmet protects you from injury.");
			did_hit = thrown;
		}
	}
	turn_off(*tp, CANFALL);
	    ...
	    && roll_em(&player, tp, weap, thrown, cur_weapon, back_stab))
	{
		...
		msg("Your %s passes right through the %s!",
	    ...
	    /*
	     * exploding freezing sphere
	     */
	    if (on(*tp, CANFEXPLODE)) {
		msg("The %s explodes.", mname);
		if (on(player, CANINWALL) || ISWEARING(R_WARMTH)) {
			msg("The explosion of the %s has no effect.", mname);
			r_know[R_WARMTH] = TRUE;
		}
		else {
			if (rnd(100) < (95 - (DISTANCE(tp->t_pos.x, tp->t_pos.y, hero.x, hero.y)*5))) {
				msg("You are frozen to death! --More--");
				wait_for(msgw, ' ');
				death(tp->t_index);
			}
			else if (rnd(100) < (70 - (DISTANCE(tp->t_pos.x, tp->t_pos.y, hero.x, hero.y)*3))) {
				msg("Your limbs stiffen.");
				no_command = FREEZETIME;
			}
			else
				msg("You escape injury.");
	     ...
	     */
	    if (on(*tp, CANFLASH)) {
		msg("The %s flashes.", mname);
		if (rnd(100) < (95 - (DISTANCE(tp->t_pos.x, tp->t_pos.y, hero.x, hero.y)*10))) {
			if (off(player, ISBLIND)) {
				msg("Everything around you becomes very dark.");
	    			turn_on(player, ISBLIND);
	    			fuse(sight, 0, rnd(30)+80, AFTER);
	    			look(FALSE, FALSE);
			}
			else
				lengthen(sight, rnd(60));
		}
		else if (rnd(100) < (70 - (DISTANCE(tp->t_pos.x, tp->t_pos.y, hero.x, hero.y)*3))) {
			if (off(player, ISBLIND)) {
				msg("Everything around you becomes dark.");
	    			turn_on(player, ISBLIND);
	    ...
	    if (on(*tp, CANFALL)) turn_off(*tp, CANFALL);
...
attack(struct thing *mp, struct object *weapon, int thrown)
{
	...
	if (!thrown) {
		...
		else if (cur_shield != NULL &&
			!cur_shield->o_flags & ISPROT) {
			msg(terse ? "Your shield weakens"
			: "Your shield appears to be weaker now. Oh my!");
			cur_shield->o_ac++;
		}
		else if (cur_helmet != NULL &&
			!cur_shield->o_flags & ISPROT) {
			msg(terse ? "Your helmet weakens"
			: "Your shield appears to be weaker now. Oh my!");
			cur_helmet->o_ac++;
		}
		else if (cur_gloves != NULL &&
			!cur_shield->o_flags & ISPROT) {
			msg(terse ? "Your gloves weaken"
			: "Your gloves appear to be weaker now. Oh my!");
			cur_gloves->o_ac++;
		}
	    ...
	    if (on(*mp, CANFALL)) turn_off(*mp, CANFALL);
		...
		if (ISWEARING(R_NOSTONE)) {
			msg("The %s's touch has no effect.", mname);
			r_know[R_NOSTONE] = TRUE;
		}
		else if (on(player, CANINWALL))
			msg("The %s's touch has no effect.", mname);
		else {
	     ...
	     * Freezing
	     */
	    if ((on(*mp, CANFREEZE)) && (no_command == 0)) {
		if (on(player, CANINWALL) || ISWEARING(R_WARMTH)) {
			msg("The %s's touch has no effect.", mname);
			r_know[R_WARMTH] = TRUE;
		}
			...
			if (rnd(100) < 10) {
				msg("You are frozen.");
				msg("You have become an ice sculpture! --More--");
				wait_for(msgw, ' ');
				death(mp->t_index);
			}
			else if (rnd(100) < 30) {
				msg("The %s's cold touch stiffens your limbs.", mname);
				no_command = FREEZETIME;
			}
		}
	   }
	   /*
	    * Xans can reduce max burden
	    */
	   if (on(*mp, CANBURDEN)) {
		extern int minus_enc;
		if ((minus_enc < 200) && (rnd(100) < 30)) {
			msg("The %s pricks your leg.", mname);
			msg("Your pack now seems heavier.");
			minus_enc += 150;
			updpack(TRUE);
		}
	    }
...
int
roll_em(struct thing *att_er, struct thing *def_er, 
struct object *weap, int hurl, 
struct object *cur_weapon, int back_stab)
{
...
void
hit(struct object *weapon, struct thing *tp, char *er, char *ee, int back_stab)
{
    register char *s = NULL;
    char
    	          att_name[80],	/* Name of attacker */
		  def_name[80];/* Name of defender */
    int see_monst = !invisible(tp);	/* Can the player see the monster? */
...
killed(struct linked_list *item, int pr, int points)
{
    ...
    if (points) {
	unsigned int test;	/* For overflow check */
