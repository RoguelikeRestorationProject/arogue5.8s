...
inv_name(struct object *obj, int drop)
{
	...
	when FOOD:
		...
		if (obj->o_count == 1)
		    ...
		    sprintf(prbuf, "%d tins", obj->o_count);
	    else if (obj->o_which == 1)
		if (obj->o_count == 1)
		    sprintf(prbuf, "A%s %s", vowelstr(fruit), fruit);
		else
		    sprintf(prbuf, "%d %ss", obj->o_count, fruit);
	    else if (obj->o_which == 2)
		if (obj->o_count == 1)
		    strcpy(prbuf, "Some food");
		else
		    sprintf(prbuf, "%d rations of food", obj->o_count);
	    else if (obj->o_which == 3)
		if (obj->o_count == 1)
		    strcpy(prbuf, "A carrot");
		else
		    sprintf(prbuf, "%d carrots", obj->o_count);
	    else if (obj->o_which == 4)
	    {
		if (obj->o_count == 1)
		    strcpy(prbuf, "A lump of royal jelly");
		else
		    sprintf(prbuf, "%d lumps of royal jelly", obj->o_count);
            }
	when WEAPON:
	    if (obj->o_count > 1)
	...
	when ARMOR:
		...
		strcat(pb, " (being worn)");
    	    else if (obj == cur_helmet)
		    strcat(prbuf, " (being worn)");
    	    else if (obj == cur_shield)
		    strcat(prbuf, " (being held)");
    	    else if (obj == cur_gloves)
		    strcat(prbuf, " (being worn)");
	when STICK:
	    sprintf(pb, "A %s%s ", 
...
dropcheck(struct object *op)
{
	...
	when R_FLOAT: msg("You float gently to the ground.");
...
new_thing(int thing_type)
{
     ...
     */
    while (thing_type == ALL && foods_this_level > 2 &&
	   j == 2)
	...
	when TYP_FOOD:
	    ...
	    if (prob < 20) /* tin */
		cur->o_which = 0;
	    else if (prob < 40) /* fruit */
		cur->o_which = 1;
	    else if (prob < 91) /* food ration */
		cur->o_which = 2;
	    else  /* carrot */
		cur->o_which = 3;
	when TYP_WEAPON:
