...
add_pack(struct linked_list *item, int silent)
{
	...
	from_floor = FALSE;
    obj = OBJPTR(item);
    if (ISWEARING(R_FLOAT) || on(player, ISFLOAT)) {
	if (obj->o_type == GOLD)
	    	msg("%s over %d gold pieces.", terse ? "Moved" : "You moved",obj->o_count);
	else
	    	msg("%s over %s", terse ? "Moved" : "You moved", inv_name(obj, TRUE));
	return(FALSE);
    }
    ...
    {
	op = OBJPTR(ip);
	if ((obj->o_type == op->o_type) && (obj->o_flags == op->o_flags) &&
    ...
    if (obj->o_flags & ISUNPAID) {
    	if ((ISWEARING(R_STEALTH) || (player.t_ctype == C_THIEF) || (pstats.s_dext > 16)) && (not_seen > 0) && in_shop && on(*shk, ISSHK)) {
		if ((rnd(100) < 20 + (pstats.s_dext-15)*10) && (not_seen < 5)) {
			msg("%s doesn't notice your activities.", shkname);
			obj->o_flags &= ~ISUNPAID;
			not_seen++;
		}
		else {
			not_paid++;
			not_seen = 0;
			msg("%s eyes you suspiciously.", shkname);
		}
	}
	else
		not_paid++;
	if ((not_paid == 3) && on(*shk, ISSHK))
		msg("%s seems nervous.", shkname);
	if ((not_paid > 3) && on(*shk, ISSHK)) {
		msg("%s loses his trust in you.", shkname);
		turn_off(*shk, ISSHK);
		turn_on(*shk, ISMEAN);
		runto(shk, &hero);
	}
...
inventory(struct linked_list *list, int type)
{
	...
	switch (n_objs++) {
		...
		if (slow_invent)
		    ...
		    if (++cnt >= MY_LINES - 2) { /* if bottom of screen */
			dbotline(hw, morestr);
			cnt = 0;
			wclear(hw);
		    }
...
pick_up(int ch)
{
	...
	case STICK:
	case RELIC:
	    while (add_pack(NULL, FALSE));	/* pick up everything there */
	    break;
    }
}
...
/* purpose = NULL if we should be silent (no prompts) */
{
	...
	if (ch == '*') {
		...
		if (++cnt >= MY_LINES - 2 && next(item) != NULL) {
		    cnt = 0;
...
cur_null(struct object *op)
{
	...
	else if (op == cur_helmet) 		cur_helmet = NULL;
	else if (op == cur_gloves) 		cur_gloves = NULL;
	else if (op == cur_shield)		cur_shield = NULL;
...
carry_obj(struct thing *mp, int chance)
{
	...
	switch (rnd(15)) {
	    ...
	    when 8: type = TRIDENT;
	    when 9: type = BASWORD;
	    when 10: type = CRYSKNIFE;
	    when 11: type = PICKAXE;
	    when 12: type = WORMTOOTH;
	    when 13: type = FLAIL;
...
grab(int y, int x)
{
	...
	for (;;) {
	    ...
	    if (ch == '*') {
		while (add_pack(NULL, TRUE));	/* pick up everything there */
		return(num_there);
	    }
	    /* ch has item to get from list */
