...
struct object *cur_gloves = NULL;
struct object *cur_helmet = NULL;
struct object *cur_shield = NULL;
...
int minus_enc = 0;
int punish_dext = 0;
int mon_know[NUMMONST+1];
int spinach_strength = 0;
int punish_str = 0;
int punished = FALSE;
int noholdweap = FALSE;
...
bool first_time;
bool not_paid;
coord shop_door, shk_pos, heavy_ball;
...
struct init_weps weaps[MAXWEAPONS] = {
    ...
    { "boomerang",	"0d0",	"2d6", NONE,		ISMETAL|ISMISL, 80, 15 },
    { "crysknife",	"1d3",  "1d4", NONE,		ISMETAL|ISMISL, 10, 2 },
    { "pick-axe", 	"1d4",	"1d2", NONE,		ISMETAL, 80, 10 },
    { "worm tooth",	"1d2",  "1d2", NONE,		ISMETAL, 80, 18 },
    { "flail",		"1d3",  "1d5", NONE,		ISMETAL|ISMISL, 50, 2 },
...
struct init_armor armors[MAXARMORS] = {
	...
	{ "studded leather armor",	30,  7,  50, 200 },
	{ "scale mail",			40,  6,  70, 250 },
	{ "padded armor",		49,  6, 150, 150 },
	{ "chain mail",			58,  5, 100, 300 },
	{ "splint mail",		68,  4, 150, 350 },
	{ "banded mail",		77,  4, 150, 350 },
	{ "plate mail",		 	83,  3, 400, 400 },
	{ "plate armor",		87,  2, 650, 450 },
	{ "shield",			90,  0, 70,  150 },
	{ "pair of gloves", 		96,  0, 50,   25 },
	{ "helmet",    			100, 0, 75,   50 },
};
...
struct magic_item s_magic[MAXSCROLLS] = {
    ...
    { "sleep",			 30, 150, 20, 0 },
    { "enchantment",		160, 200, 9, 9 },
    { "identify",		170, 100, 0, 25 },
    { "scare monster",		 30, 250, 27, 21 },
...
struct magic_item r_magic[MAXRINGS] = {
    ...
    { "fear",			 20, 100, 100, 0},
    { "heroism",		 30, 390, 0, 0 },
    { "fire resistance",	 35, 400, 0, 0 },
    { "warmth",	 		 35, 400, 0, 0 },
    { "vampiric regeneration",	 10,1000, 0, 0},
    { "stone resistance",	 30, 700, 0, 0},
    { "levitation",		 40, 300, 0, 0},
};

struct magic_item ws_magic[MAXSTICKS] = {
    { "light",			 80, 120, 20, 20 },
    { "striking",		 50, 115, 0,  0 },
    { "lightning",		 30, 200, 0,  0 },
    { "fire",			 30, 200, 0,  0 },
    { "cold",			 30, 200, 0,  0 },
    { "polymorph",		 70, 150, 0,  0 },
    { "magic missile",		 70, 170, 0,  0 },
    { "slow monster",		 70, 220, 25, 20 },
    { "drain life",		 80, 210, 20, 0 },
    { "charging",		 70, 400, 0,  0 },
    { "teleport monster",	 80, 140, 25, 20 },
    { "cancellation",		 30, 130, 0,  0 },
    { "confuse monster",   	 30, 100, 15,  0},
    ...
    { "wonder",			 40, 110,  0, 0},
    { "fear",			 30, 180,  0, 0},
    { "make invisible",		 40, 140,  0, 0},
    { "death",			 20, 300,  0, 0},
    { "sleep",			 30, 200,  0, 0},
    { "undead affecting",	 30, 180,  0, 0},
};
...
{"orc",
		15,	TRUE,	TRUE,	'O',	"8",
		{ISMEAN, CANSHOOT, CARRYGOLD, CARRYWEAPON, INPACK},
		0, 0,
		1,
		{12,	10,	1,	6,	HPT("1d8"),
		"1d8"}},
...
{"gas spore",
		0,	TRUE,	FALSE,	'a',	"0",
		{ISMEAN, CANEXPLODE, CANINFEST, ISFLY, NOSTAB},
		0, 0,
		5,
		{10,	90,	1,	9,	HPT("1d1"),
		"1d1"}},
...
{"hobgoblin",
		10,	TRUE,	TRUE,	'h',	"8-10",
		{ISMEAN, CANSHOOT, CARRYWEAPON, INPACK},
		0, 0,
		2,
		{14,	20,	1,	5,	HPT("1d8+1"),
		"1d8"}},
...
{"ghoul",
		0,	TRUE,	TRUE,	'g',	"5-7",
		{ISMEAN, CANPARALYZE, ISUNDEAD, TURNABLE, INPACK},
		0, 0,
		2,
		{10,	65,	2,	6,	HPT("2d8"),
		"1d3/1d3/1d6"}},
...
{"jaguar",
		...
		0, 0,
		5,
		{18,	120,	4,	5,	HPT("4d8+1"),
		"2d8"}},
{"freezing sphere",
		50,	TRUE,	FALSE,	'f',	"0-0",
		{NOMOVE, CANFREEZE, CANFEXPLODE},
		0, 0,
		5,
		{10,	100,	4,	8,	HPT("2d8"),
		"2d4"}},
...
{"shadow",
		0,	TRUE,	TRUE,	'S',	"5-7",
		{ISSHADOW, ISMEAN, CANCHILL, ISUNDEAD, TURNABLE, NOSTAB},
		0, 0,
		4,
...
{"medusa",
		5,	TRUE,	TRUE,	'M',	"7-9",
		{ISMEAN, LOOKSTONE, TOUCHSTONE, CANSNORE},
		0, 0,
		8,
		{14,	375,	5,	4,	HPT("5d8"),
		"2d8"}},
...
{"fog cloud",
		35,	TRUE,	FALSE,	'f',	"0-0",
		{ISMEAN, CANDISEASE, CANINFEST, NOSTAB},
		0, 0,
		5,
		{10,	150,	4,	-1,	HPT("2d3"),
		"0d0"}},
{"troll",
		50,	TRUE,	FALSE,	'T',	"5-7",
		{ISMEAN, ISREGEN, CARRYFOOD, CARRYGOLD, CARRYSTICK, INPACK},
		0, 0,
		8,
...
{"quivering blob",
		35,	TRUE,	FALSE,	'Q',	"0-0",
		{NOMOVE, CANHUG, CANHOLD, CANSUFFOCATE},
		0, 0,
		5,
		{13,	540,	4,	7,	HPT("4d8+3"),
		"2d4"}},
{"wraith",
		0,	TRUE,	TRUE,	'w',	"11-12",
		{ISMEAN, CANDRAIN, MAGICHIT, ISUNDEAD, TURNABLE},
...
{"mummy",
		...
		{ISMEAN,CANINFEST, MAGICHIT, CANFRIGHTEN, HALFDAMAGE, ISUNDEAD, 
		 TURNABLE, CARRYRING, CARRYSTICK, INPACK},
		0, 0,
		8,
...
{"vampire",
		...
		{ISMEAN, ISREGEN, CANSUCK, ISUNDEAD, TURNABLE, CARRYMISC, INPACK},
		0, 0,
		12,
...
{"quartermaster", 
		...
		{CANSELL, CARRYPOTION, CARRYSCROLL, CARRYMISC, CARRYRING,
		 CARRYSTICK, CANTELEPORT,BMAGICHIT,MAGICHIT,ISREGEN,NOFIRE,NOCOLD},
		0, 0,
		2,
		{30,	20,	1,	-11, 	HPT("0d8+760"),
		"12d10"}},
};
